...
#### 自定义导航栏
```vue
<custom-nav-bar title="标题"></custom-nav-bar>
```

#### 通用标题组件
```vue
<common-title>
  <template #name>标题</template>
  <template #custom>自定义内容</template>
</common-title>
```
...
```javascript
import { apiGetBannr } from '@/api/apis.js'
